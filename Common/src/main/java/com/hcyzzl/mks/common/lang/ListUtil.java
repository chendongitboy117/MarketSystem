package com.hcyzzl.mks.common.lang;
...
public class ListUtil {
    ...
    /**
     * 获取两个集合中的交集
     * 比较相等性是使用参数中的 {@param comparator} 函数，不依赖于 {@link T#hashCode()} 方法
     * 注意：该方法的时间复杂度为 On^2
     *
     * @param left  第一个集合
     * @param right 第二个集合
     ...
     * @return 两个集合的交集
     */
    public static <T> Set<T> intersection(List<T> left, List<T> right, Comparator<T> comparator) {
        return left.stream().filter(l -> right.stream().anyMatch(r -> comparator.compare(l, r) == 0)).collect(Collectors.toSet());
    }
     ...
     * @return 差异，主要分为 {@param left} 独有元素，{@param right} 独有元素以及公共元素
     */
    public static <T> Map<ListDiffState, List<T>> different(List<T> left, List<T> right, Comparator<T> comparator) {
        final List<T> commonList = new ArrayList<>(intersection(left, right, comparator));
        final List<T> leftExclusive = filter(left, right, comparator);
        final List<T> rightExclusive = filter(right, left, comparator);
        final HashMap<ListDiffState, List<T>> map = new HashMap<>(3);
        map.put(ListDiffState.left, leftExclusive);
        map.put(ListDiffState.right, rightExclusive);
        map.put(ListDiffState.common, commonList);
     ...
     * @return 差异，主要分为 {@param left} 独有元素，{@param right} 独有元素以及公共元素
     */
    public static <T> Map<ListDiffState, List<T>> different(List<T> left, List<T> right, BiFunction<T, T, Boolean> comparator) {
        return different(left, right, (Comparator<T>) (a, b) -> comparator.apply(a, b) ? 0 : 1);
    }
